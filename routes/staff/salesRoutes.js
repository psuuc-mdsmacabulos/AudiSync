...
// Endpoint 1: Overview Statistics (Total Sales and Orders Only)
...
// Endpoint 2: Chart Data (Sales and Orders by Period)
router.get("/statistics", authMiddleware, async (req, res) => {
  try {
    ...
    // Combine periods from sales and orders
    const periods = new Set([
      ...salesData.map((entry) => entry.period),
      ...ordersData.map((entry) => entry.period),
    ]);

    // Build chart data for all months if interval is monthly
    let chartData = [];
    if (interval === "monthly") {
...
// Endpoint 3: Revenue Data (Repurposed from /profits)
...
// Endpoint 5: Orders Dashboard Data
